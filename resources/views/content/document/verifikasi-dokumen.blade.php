...
<div class="container">
    <div class="row">
        <!-- Kolom Kiri: Preview File + Verifikasi -->
        <div class="col-md-8">
            @php
                ...
                @if($isPdf)
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        ...
                        <div class="me-2">
                            ...
                            <button onclick="window.history.back()" class="btn btn-outline-info btn-sm">⬅ Kembali</button>
                        </div>
                    </div>
            ...
            <form action="{{ route('documents.updateStatusVerifikasi', $document->id) }}" method="POST">
                ...
                <div class="mt-4 card p-3 shadow-sm">
                    ...
                    <textarea name="catatan_admin" class="form-control mb-3" rows="4" placeholder="Tuliskan catatan di sini...">{{ old('catatan_admin', $document->catatan_verifikasi ?? '') }}</textarea>

                    <div class="d-flex gap-2">
                        <button type="submit" name="status_verifikasi" value="0" class="btn btn-secondary">❌ Batal</button>
                        <button type="submit" name="status_verifikasi" value="2" class="btn btn-success">✅ Verifikasi</button>
                        <button type="submit" name="status_verifikasi" value="3" class="btn btn-warning text-white">⚠️ Butuh Perbaikan</button>
                    </div>
                </div>
            </form>
        </div>
                ...
                @foreach ($fields as $label => $value)
                    ...
                    @if($showItem)
                        <li class="list-group-item">
                            <strong>{{ $label }}:</strong>
                            @if ($label === 'Dokumen Perubahan')
                                <a href="{{ asset('storage/' . $value->pdf_file) }}" target="_blank">{{ $value->judul }}</a>
                            @else
                                {{ $value }}
                            @endif
                        </li>
...
<script>
    ...
    pdfjsLib.getDocument(url).promise.then(pdf=>{ pdfDoc=pdf; renderPage(pageNum); });
</script>
@endif
@endsection
